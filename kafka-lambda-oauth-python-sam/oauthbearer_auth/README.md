...
- `kafka_json_apps` - Standalone Python producer/consumer sample apps (Faker JSON, kafka-python) used from the client EC2 instance.
...
Wait for the stack to reach `CREATE_COMPLETE`. It creates a VPC (1 public / 3 private subnets, NAT), 3 Kafka broker EC2 instances, an Amazon Cognito User Pool (+ domain, resource server, `client_credentials` app client, and three users), and a client EC2 instance with Java, Python 3, Docker, the AWS CLI, the AWS SAM CLI, Kafka CLI tools, the producer/consumer apps and the helper scripts installed. (Java is still present because the Kafka CLI tools used by the admin/negative scripts are part of Apache Kafka and run on the JVM; the Lambda function and the producer/consumer sample apps are Python.)
