...
* **OAuth authentication**: The broker client listener uses `SASL_SSL` with the `OAUTHBEARER` mechanism, implemented with the [Strimzi kafka-oauth](https://github.com/strimzi/strimzi-kafka-oauth) libraries. Brokers validate incoming JWT access tokens against the Cognito User Pool's JWKS endpoint (`https://cognito-idp.<region>.amazonaws.com/<userPoolId>/.well-known/jwks.json`). TLS uses a self-signed certificate (with the broker IPs as SANs) generated by broker 1 and shared to the other brokers and the client via the S3 cache; clients trust it through a PKCS12 truststore.
...
> **Important — OAuth event source support:** AWS **SAM does not yet support** the `OAUTHBEARER` auth type for self-managed Apache Kafka event sources, so the `SelfManagedKafka` event in `template_original.yaml` is scaffolding (VPC config + a placeholder auth entry) and `sam deploy` will not create a working mapping. The OAuth event source **does** work through the **AWS CLI** — see the next section.

## Deploy the Lambda consumer via the AWS CLI (OAuth)

`deploy_lambda_oauth_cli.sh` deploys the same Java consumer with a self-managed Kafka event source using `aws lambda create-event-source-mapping`, wiring the `OAUTHBEARER_AUTH` (client-credentials secret), `OAUTHBEARER_SCOPE`, `SERVER_ROOT_CA_CERTIFICATE`, VPC subnet/SG, and the required `provisioned-poller-config`. It auto-discovers the VPC config and the Cognito `client_credentials` app client (`PollerClientId`) from the stack, fetches the app-client secret, and uses the broker cert the client fetched (`/home/ec2-user/kafka.crt`) as the TLS trust anchor.

Requirements: your account must be **allowlisted** for the new event-source auth types, the brokers must expose the **SASL_SSL/OAUTHBEARER** listener (this stack does, on `:9092`), and the Cognito `client_credentials` app client + domain + scope must exist (this stack creates them).

Run it from the pattern directory on the client EC2 instance:
...
Then watch the mapping and produce test messages:
...
```
Check the function's CloudWatch Logs for the consumed messages.
